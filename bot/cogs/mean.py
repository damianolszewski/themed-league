...
class Mean(commands.Cog):
    ...
    async def mean(self, interaction: discord.Interaction):
        try:
            ...
            # This library supports multiple speech engines, including Google, Microsoft, and Apple
            engine = pyttsx3.init()
            engine.setProperty("voice", "polish")
            engine.setProperty("rate", 150)
            engine.setProperty("volume", 1.0)

            # Create a stream object from the audio bytes generated by the speech engine
            stream = io.BytesIO()
            engine.save_to_file(compliment, stream)
            stream.seek(0)
